...
import { uniNavigation } from "@/lib/navigation"
...
export function UniversitySidebar() {
    ...
    return (
        <AppSidebar
            ...
            primary={uniNavigation.primary}
            secondary={uniNavigation.secondary}
            secondaryLabel="Management"
            isCollapsed={isCollapsed}
            setIsCollapsed={setIsCollapsed}
            isMobileOpen={isMobileOpen}
            setIsMobileOpen={setIsMobileOpen}
            user={session?.user ? { name: session.user.name, image: session.user.image, role: (session.user as { role?: string }).role ?? "Faculty" } : null}
            isPending={isPending}
