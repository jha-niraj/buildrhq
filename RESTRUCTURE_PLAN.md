# BuildrHQ — Monorepo Restructure Plan
...
`buildrhq` is already a **Turborepo + pnpm** monorepo. Stack: Next.js 15.4 (App
...
| `coderzworker` | — | Express + BullMQ + Dockerode code-execution worker (not Next.js) | ok |
...
```
apps/main/app/
├── page.tsx                 ← MARKETING landing (BuildrHQ hero/features/pricing)
...
|---|---|---|---|
| **`web`** *(new)* | `buildrhq.com` | 3004 | Marketing landing, about, blogs, legal, pricing, SEO. **No auth, no middleware, no DB writes.** Minimal deps. |
| `main` | `app.buildrhq.com` | 3000 | The authenticated product: `(auth)`, `(main)`, `(jobs)`, `api`. Behind middleware + layout guards. |
| `uni` | `uni.buildrhq.com` | 3001 | (unchanged) |
| `hiring` | `hiring.buildrhq.com` | 3002 | (unchanged) |
| `admin` | `admin.buildrhq.com` | 3003 | (unchanged) |
| `coderzworker` | (internal) | — | (unchanged) |
...
3. **Cross-subdomain better-auth cookies** — add `advanced.crossSubDomainCookies`
   + `cookieDomain: .buildrhq.com` in `packages/auth` so a session set on
   `app.buildrhq.com` is readable by `web` (lets the marketing navbar show
...
- Add `advanced: { crossSubDomainCookies: { enabled: true, domain: process.env.AUTH_COOKIE_DOMAIN }, cookiePrefix: "buildrhq", useSecureCookies: <prod> }`.
...
- **Phase 4 — SEO** — move/author `sitemap.ts`, `robots.ts`, add `manifest.ts`,
  `llms.txt`, JSON-LD. Point canonical URLs at `buildrhq.com`.
...
1. **Production domains.** Assumed `buildrhq.com` (web) + `app.buildrhq.com`
