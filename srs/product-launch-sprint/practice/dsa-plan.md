...
> Worker: `apps/shipitworker/` (Docker-based code execution)
...
4. Student writes code → ShipItHQWorker executes it → AI evaluates output + code together
...
- [ ] **Conversation flow states** - The panel has distinct phases:
  ...
  CODE_SUBMITTED           → AI waits for ShipItHQWorker result
...
- [ ] **Create `assess.action.ts`** `evaluateDSAApproach(sessionId, userMessage, phase, codeOutput?)`:
  ...
  - If phase is `CODE_SUBMITTED`: run code through ShipItHQWorker (see A4). Pass output to AI.
...
#### A4 - Code Execution via ShipItHQWorker

- [ ] **"Run Code" button** in the workspace → sends code + language to ShipItHQWorker.
...
3. Code execution integration - wire "Run Code" button to ShipItHQWorker + show test results
