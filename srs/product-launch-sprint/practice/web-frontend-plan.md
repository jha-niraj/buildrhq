...
- `CODING` - Write JavaScript/TypeScript code (evaluated by ShipItHQWorker + AI)
...
- `DEBUG` - Fix broken code (evaluated by ShipItHQWorker + AI)
...
- [ ] **`evaluateFrontendCode()` action**:
  - For CODING problems: run code through ShipItHQWorker (Node.js runner for JS/TS). Check against test cases. Also pass code to AI for quality evaluation (readability, edge cases, browser compatibility, performance).
  ...
  - For DEBUG problems: run the fixed code through ShipItHQWorker. Check if it passes the test cases that were failing.
...
- [ ] **Console output panel**: For JS problems, show console.log output without needing a full browser. Use ShipItHQWorker to execute and capture stdout.
